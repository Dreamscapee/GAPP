...
def setupCalc(username, password, weather, sessionTemp):
    ...
    browser.submit()
    browser2.open("https://www.gpro.net/gb/Login.asp")
    browser2.select_form(id="Form1")
    browser2.form["textLogin"] = username
    browser2.form["textPassword"] = password
    browser2.submit()
    ...
    driverAggressiveness = int(
        tree.xpath("normalize-space(//td[contains(@id, 'Aggr')]/text())")
    )
    driverExperience = int(
        tree.xpath("normalize-space(//td[contains(@id, 'Experience')]/text())")
    )
    driverTechnicalInsight = int(
        tree.xpath("normalize-space(//td[contains(@id, 'TechI')]/text())")
    )
    driverWeight = int(
        tree.xpath("normalize-space(//tr[contains(@data-step, '14')]//td/text())")
    )
    ...
    tree = html.fromstring(browser.response().get_data())
    browser.back()
    browser2.follow_link(url_regex=re.compile("Testing"))
    browser2.follow_link(url_regex=re.compile("TrackDetails"))
    treeTest = html.fromstring(browser2.response().get_data())
    browser2.back()
    browser2.back()
    trackName = str(
    ...
    testTrackName = str(
        treeTest.xpath("normalize-space(//h1[contains(@class, 'block')]/text())")
    )
    testTrackName = testTrackName.strip()
    ...
    browser.back()
    browser2.follow_link(url_regex=re.compile("Testing"))
    treeTestW = html.fromstring(browser2.response().get_data())
    rTempRangeOne = str(
        tree.xpath(
            "normalize-space(//td[contains(text(), 'Temp')]/../../tr[2]/td[1]/text())"
        )
    )
    rTempRangeTwo = str(
        tree.xpath(
            "normalize-space(//td[contains(text(), 'Temp')]/../../tr[2]/td[2]/text())"
        )
    )
    rTempRangeThree = str(
        tree.xpath(
            "normalize-space(//td[contains(text(), 'Temp')]/../../tr[4]/td[1]/text())"
        )
    )
    rTempRangeFour = str(
        tree.xpath(
            "normalize-space(//td[contains(text(), 'Temp')]/../../tr[4]/td[2]/text())"
        )
    )
    qTesting = str(
        treeTestW.xpath(
            # "normalize-space(//img[contains(@title, 'Partially Cloudy')]/../text()[contains(., 'Temp')])"
            "normalize-space(//img[contains(@align, 'absmiddle')]/../text()[contains(., 'Temp')])"
        )
    )
    ...
    rTemp = (
        (rTempMinOne + rTempMaxOne)
        + (rTempMinTwo + rTempMaxTwo)
        + (rTempMinThree + rTempMaxThree)
        + (rTempMinFour + rTempMaxFour)
    ) / 8
    # Using the race strategy page requested earlier, scrape the qualifying weather data
    qOneTemp = str(
        tree.xpath(
            "normalize-space(//img[contains(@name, 'WeatherQ')]/../text()[contains(., 'Temp')])"
        )
    )
    qOneTemp = int((re.findall(r"\d+", qOneTemp))[0])
    qTwoTemp = str(
        tree.xpath(
            "normalize-space(//img[contains(@name, 'WeatherR')]/../text()[contains(., 'Temp')])"
        )
    )
    qTwoTemp = int((re.findall(r"\d+", qTwoTemp))[0])

    qTesting = int((re.findall(r"\d+", qTesting))[0])
    ...
    if sessionTemp == "Race":
        ...
        trackN = trackName
    elif sessionTemp == "Testing":
        sessionTemp = qTesting
        trackN = testTrackName
    # Check the user selected session and assign the relevant track
    ...
    carWearEngine = str(
        tree.xpath(
            r"normalize-space(//b[contains(text(), 'Engine')]/../../td[4]/text())"
        )
    )
    if carWearEngine == "":
        carWearEngine = str(
    ...
    carWearFrontWing = str(
        tree.xpath(
            r"normalize-space(//b[contains(text(), 'Front wing')]/../../td[4]/text())"
        )
    )
    if carWearFrontWing == "":
        carWearFrontWing = str(
    ...
    carWearRearWing = str(
        tree.xpath(
            r"normalize-space(//b[contains(text(), 'Rear wing')]/../../td[4]/text())"
        )
    )
    if carWearRearWing == "":
        carWearRearWing = str(
    ...
    carWearUnderbody = str(
        tree.xpath(
            r"normalize-space(//b[contains(text(), 'Underbody')]/../../td[4]/text())"
        )
    )
    if carWearUnderbody == "":
        carWearUnderbody = str(
    ...
    carWearSidepod = str(
        tree.xpath(
            r"normalize-space(//b[contains(text(), 'Sidepods')]/../../td[4]/text())"
        )
    )
    if carWearSidepod == "":
        carWearSidepod = str(
    ...
    carWearCooling = str(
        tree.xpath(
            r"normalize-space(//b[contains(text(), 'Cooling')]/../../td[4]/text())"
        )
    )
    if carWearCooling == "":
        carWearCooling = str(
    ...
    carWearGears = str(
        tree.xpath(
            r"normalize-space(//b[contains(text(), 'Gearbox')]/../../td[4]/text())"
        )
    )
    if carWearGears == "":
        carWearGears = str(
    ...
    carWearSuspension = str(
        tree.xpath(
            r"normalize-space(//b[contains(text(), 'Suspension')]/../../td[4]/text())"
        )
    )
    ...
    carWearElectronics = str(
        tree.xpath(
            r"normalize-space(//b[contains(text(), 'Electronics')]/../../td[4]/text())"
        )
    )
    ...
    trackBaseWingsSetup = float(trackData[trackN][0]) * 2
    trackBaseEngineSetup = float(trackData[trackN][1])
    trackBaseBrakesSetup = float(trackData[trackN][2])
    trackBaseGearsSetup = float(trackData[trackN][3])
    trackBaseSuspensionSetup = float(trackData[trackN][4])
    trackBaseWingSlitSetup = float(trackData[trackN][5])
    ...
    driverOffsets = [[0.3], [-0.5], [0.5], [0.75, 2]]
    ...
    if weather != "WET":
        ...
        setupWeather = (
            (baseOffsets["wingWeatherWet"] * sessionTemp)
            + baseOffsets["wingWeatherOffset"]
        ) * 2
    setupDriver = (
        driverTalent
        * (trackBaseWingsSetup + setupWeather)
        * baseOffsets["wingDriverMultiplier"]
    )
    setupCarLevel = (
        (carLevelOffsets[0][0] * carLevelChassis)
        + (carLevelOffsets[0][1] * carLevelFrontWing)
        + (carLevelOffsets[0][1] * carLevelRearWing)
        + (carLevelOffsets[0][2] * carLevelUnderbody)
    )
    setupCarWear = (
        (carWearOffsets[0][0] * carWearChassis)
        + (carWearOffsets[0][1] * carWearFrontWing)
        + (carWearOffsets[0][1] * carWearRearWing)
        + (carWearOffsets[0][2] * carWearUnderbody)
    )
    setupWings = (
        trackBaseWingsSetup + setupWeather + setupDriver + setupCarLevel + setupCarWear
    ) / 2

    # Wing Split
    if weather != "WET":
        setupWingSplit = (
            trackBaseWingSlitSetup
            + (driverTalent * -0.246534498671854)
            + (3.69107049712848 * (carLevelFrontWing + carLevelRearWing) / 2)
            + (setupWings * -0.189968386659174)
    ...
    if weather != "WET":
        ...
        setupWeather = (baseOffsets["engineWeatherWet"] * sessionTemp) + baseOffsets[
            "engineWeatherOffset"
        ]
    setupDriver = (driverOffsets[0][0] * driverAggressiveness) + (
        driverExperience
        * (
            (
                (trackBaseEngineSetup + setupWeather)
                * baseOffsets["engineDriverMultiplier"]
            )
            + baseOffsets["engineDriverOffset"]
        )
    )
    setupCarLevel = (
        (carLevelOffsets[1][0] * carLevelEngine)
        + (carLevelOffsets[1][1] * carLevelCooling)
        + (carLevelOffsets[1][2] * carLevelElectronics)
    )
    setupCarWear = (
        (carWearOffsets[1][0] * carWearEngine)
        + (carWearOffsets[1][1] * carWearCooling)
        + (carWearOffsets[1][2] * carWearElectronics)
    )
    setupEng = (
        trackBaseEngineSetup + setupWeather + setupDriver + setupCarLevel + setupCarWear
    )
    ...
    if weather != "WET":
        ...
        setupWeather = (baseOffsets["brakesWeatherWet"] * sessionTemp) + baseOffsets[
            "brakesWeatherOffset"
        ]
    setupDriver = driverOffsets[1][0] * driverTalent
    setupCarLevel = (
        (carLevelOffsets[2][0] * carLevelChassis)
        + (carLevelOffsets[2][1] * carLevelBrakes)
        + (carLevelOffsets[2][2] * carLevelElectronics)
    )
    setupCarWear = (
        (carWearOffsets[2][0] * carWearChassis)
        + (carWearOffsets[2][1] * carWearBrakes)
        + (carWearOffsets[2][2] * carWearElectronics)
    )
    setupBra = (
        trackBaseBrakesSetup + setupWeather + setupDriver + setupCarLevel + setupCarWear
    )
    ...
    if weather != "WET":
        ...
        setupWeather = (baseOffsets["gearsWeatherWet"] * sessionTemp) + baseOffsets[
            "gearsWeatherOffset"
        ]
    setupDriver = driverOffsets[2][0] * driverConcentration
    setupCarLevel = (carLevelOffsets[3][0] * carLevelGears) + (
        carLevelOffsets[3][1] * carLevelElectronics
    )
    setupCarWear = (carWearOffsets[3][0] * carWearGears) + (
        carWearOffsets[3][1] * carWearElectronics
    )
    setupGea = (
        trackBaseGearsSetup + setupWeather + setupDriver + setupCarLevel + setupCarWear
    )
    ...
    if weather != "WET":
        ...
        setupWeather = (
            baseOffsets["suspensionWeatherWet"] * sessionTemp
        ) + baseOffsets["suspensionWeatherOffset"]
    if weather != "WET":
        setupDriver = (driverOffsets[3][0] * driverExperience) + (
            driverOffsets[3][1] * driverWeight
        )
    else:
        setupDriver = (
            (driverOffsets[3][0] * float(driverExperience))
            + (driverOffsets[3][1] * driverWeight)
            + (driverTechnicalInsight * 0.11)
        )
    setupCarLevel = (
        (carLevelOffsets[4][0] * carLevelChassis)
        + (carLevelOffsets[4][1] * carLevelUnderbody)
        + (carLevelOffsets[4][2] * carLevelSidepod)
        + (carLevelOffsets[4][3] * carLevelSuspension)
    )
    setupCarWear = (
        (carWearOffsets[4][0] * carWearChassis)
        + (carWearOffsets[4][1] * carWearUnderbody)
        + (carWearOffsets[4][2] * carWearSidepod)
        + (carWearOffsets[4][3] * carWearSuspension)
    )
    setupSus = (
        trackBaseSuspensionSetup
        + setupWeather
        + setupDriver
        + setupCarLevel
        + setupCarWear
...
def strategyCalc(username, password, minimumWear, laps):
    ...
    trackTyreWearRating = str(
        tree.xpath(
            "normalize-space(//td[contains(text(), 'Tyre wear')]/following-sibling::td/text())"
        )
    )
    trackLaps = str(
        tree.xpath("normalize-space(//td[contains(text(), 'Laps:')]/../td[2]/text())")
    )
    trackLapsInt = float(re.search("\d+", trackLaps).group())
    trackLapDistance = str(
        tree.xpath(
            "normalize-space(//td[contains(text(), 'Lap distance:')]/../td[2]/text())"
        )
    )
    ...
    trackDistance = str(
        tree.xpath(
            "normalize-space(//td[contains(text(), 'Race distance:')]/../td[2]/text())"
        )
    )
    try:
    ...
    trackPitInOut = str(
        tree.xpath(
            "normalize-space(//td[contains(text(), 'Time in/out of pits:')]/../td[2]/text())"
        )
    )
    try:
    ...
    try:
        ...
        tdExperience = int(
            tree.xpath("//th[contains(text(), 'Experience:')]/../td[0]/text()")[0]
        )
        tdPitCoordination = int(
            tree.xpath("//th[contains(text(), 'Pit coordination:')]/../td[0]/text()")[0]
        )
        technicalDirectorValues = [
            0.0314707991001518,
            -0.0945456184596369,
            -0.0355383420267692,
            -0.00944695128810026,
    ...
    staffConcentration = int(
        tree.xpath("//th[contains(text(), 'Concentration:')]/../td/text()")[0]
    )
    staffStress = int(
        tree.xpath("//th[contains(text(), 'Stress handling:')]/../td/text()")[0]
    )
    ...
    for i in range(4):
        stops.append(
            int(
                stopCalc(
                    trackDistanceFloat,
                    trackWearLevel[trackTyreWearRating],
                    rTemp,
                    tyreSupplierFactor[tyreSupplierName],
                    i,
                    carLevelSuspension,
                    driverAggressiveness,
                    driverExperience,
                    driverWeight,
                    float(trackData[trackName][9]),
                    minimumWear,
                    wearFactors[i],
                    1,
                )
    ...
    fuelFactor = (
        (-0.000101165467155397 * driverConcentration)
        + (0.0000706080613787091 * driverAggressiveness)
        + (-0.0000866455021527332 * driverExperience)
        + (-0.000163915452803369 * driverTechnicalInsight)
        + (-0.0126912680856842 * carLevelEngine)
        + (-0.0083557977071091 * carLevelElectronics)
    )
    fuels = []
    for i in range(4):
        fuels.append(
        ...
        pitTimes.append(
            round(
                float(
                    pitTimeCalc(
                        int(fuels[i]),
                        technicalDirectorValues[0],
                        technicalDirectorValues[1],
                        staffConcentration,
                        technicalDirectorValues[2],
                        staffStress,
                        technicalDirectorValues[3],
                        tdExperience,
                        technicalDirectorValues[4],
                        tdPitCoordination,
                        0,
                    )
                ),
    ...
    for i in range(4):
        pitTotals.append(
            round(
                (float(stops[i]) * (float(pitTimes[i]) + float(trackPitInOutFloat))), 2
            )
        )
    ...
    FLDs.append(
        round(
            fuelTimeCalc(
                trackDistanceFloat, trackData[trackName][7], fuelFactor, stops[4] + 1
            ),
            2,
        )
    )
    ...
    TCDs[1] = round(
        compoundCalc(
            trackLapsInt,
            float(trackData[trackName][11]),
            trackLapDistanceFloat,
            rTemp,
            tyreCompoundSupplierFactor[tyreSupplierName],
        ),
        2,
    )
    ...
    if fuels[4] <= 0:
        ...
        for i in range(3):
            totals.append(
                totalTimeCalc(
                    float(pitTotals[i + 1]), float(TCDs[i + 1]), float(FLDs[i + 1])
                )
            )
...
def stopCalc(
    ...
    productFactors = (
        (0.896416176238624**trackWearLevel)
        * (0.988463622**rTemp)
        * (1.048876356**tyreSupplierFactor)
        * (1.355293715**tyreType)
        * (1.009339294**carLevelSuspension)
        * (0.999670155**driverAggressiveness)
        * (1.00022936**driverExperience)
    ...
    stops = (
        math.ceil(
            (trackDistanceTotal)
            / (
                (productFactors * baseWear * trackBaseWear * wetFactor)
                * ((100 - wearLimit) / 100)
            )
        )
        - 1
...
def customLapFuelLoadCalc(
    trackDistanceTotal, trackFuelBase, fuelFactor, trackLapsCount, laps
):
    fuelLoad = (
        laps * (trackDistanceTotal * (trackFuelBase + fuelFactor)) / trackLapsCount
    )
    return fuelLoad
...
def pitTimeCalc(
    fuelLoad,
    tdInfluenceFuel,
    tdInfluenceStaffConcentration,
    staffConcentration,
    tdInfluenceStaffStress,
    staffStress,
    tdInfluenceExperience,
    tdExperience,
    tdInfluencePitCoordination,
    tdPitCoordination,
    pitInOut,
):
    return round(
        (
            (fuelLoad * tdInfluenceFuel)
            + 24.26
            + (tdInfluenceStaffConcentration * staffConcentration)
            + (tdInfluenceStaffStress * staffStress)
            + (tdInfluenceExperience * tdExperience)
            + (tdInfluencePitCoordination * tdPitCoordination)
            + pitInOut
        ),
        2,
    )
...
def fuelTimeCalc(trackLapsCount, trackFuelBase, fuelFactor, stints):
    return 0.0025 * (
        (trackLapsCount * trackLapsCount * (trackFuelBase + fuelFactor)) / stints
    )
...
def compoundCalc(
    ...
    return trackLapsCount * (
        (trackCornerCount * trackDistanceLap * 0.00018 * (50 - rTemp))
        + tyreCompoundSupplierFactor
    )
...
def wearCalc(startWear, partLevel, driverFactor, trackName, clearTrackRisk, i):
    ...
    return (
        wearData[trackName][i]
        * (levelFactors[partLevel - 1] ** clearTrackRisk)
        * driverFactor
    )
